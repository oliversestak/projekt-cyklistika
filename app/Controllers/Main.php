...
class Main extends BaseController
{
    public function stranka1()
    {
        ...
        $stages = $stageModel->where('id_race_year', $raceYear['id'])->orderBy('number', 'asc')->findAll();
    ...
    public function etapa($id)
    {
        ...
        $results = $resultModel
            ...
            ->join('cyklistika_rider', 'cyklistika_result.id_rider = cyklistika_rider.id')
            ->where(['id_stage' => $id, 'type_result' => 'GC'])
            ->orderBy('rank', 'asc')
            ->findAll();

        return view('etapa_detail', ['stage' => $stage, 'results' => $results]);
    }
